...
function array(){ // 매개변수 x시 arguments라는 배열이 생김.  실행시의 매개변수가 들어옴.
...
// } arguments.length마다 다른 명령어를 입력하여 상황에 따라 다르게 반응하는 함수로 만듬.
    function array(){
        ...
        if (arguments.length==0){
            return result;
        } else if (arguments.length==1){ // length가 arguments[0]인 배열을 만듬. arguments[0]값이 3이라면, array(3)을 만드는 경우 빈 방이 3개 만들어지는 것과 동일.
            for (var cnt = 0; cnt <=arguments[0]; cnt++){
                result.push(null); // 방은 비어있고, 만들어지는 갯수는 arguments[0] (첫번째 매개변수) 만큼.
              
            }   return result;           
        } else if (arguments.length==2){ // arguments[0], arguments[1]...을 배열로 만듬. (array(1,2))
            for (var idx= 0; idx <arguments.length; idx++){
                result.push(arguments[idx]); // 1방값, 2방값... 매개변수 갯수만큼 배열에 매개변수 값을 넣어줌.
            }   return result;
        } else if (arguments.length=3){ // 2와 마찬가지.
            for (var idx=0; idx <arguments.length; idx++){
