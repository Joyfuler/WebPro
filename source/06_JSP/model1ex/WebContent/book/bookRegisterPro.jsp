...
<%
		...
		for (String fileNames : images){
			...
			try{
				...
				if(serverFile.exists()){
					...
					os = new FileOutputStream("D:\\webPro\\source\\06_JSP\\model1ex\\WebContent\\bookImg\\"
	...
	String bimage1 = images[1] == null? "noImg.png" : images[1];// 배열에 들어갈 때는 반대로 들어가므로 순서를 반대로 해줘야
	String bimage2 = images[0] == null? "NOTHING.JPG" : images[0];
	// 파일명만 가져온 것이므로 경로필요x
	String bcontent = mRequest.getParameter("bcontent");
	int bdiscount = Integer.parseInt(mRequest.getParameter("bdiscount"));	
	// DB 저장
	BookDto dto = new BookDto(0, btitle, bprice, bimage1, bimage2, bcontent, bdiscount, null); // 넣을 책 정보를 담은 객체를 생성하기
...
%>
 ...
 <%if (bdiscount!=0){ %>
 <h3> 책가격 <del><%=bprice%></del>
 <b><%=finalPrice%>원(<%=bdiscount%>% 할인)</b></h3>
 <%} else{ %>
 <h3> 책가격 <b><%=bprice%>원</b></h3>
 <%}%>
 <h3>대표이미지: <%=path%>/<%=bimage1%></h3>
 <h3>추가이미지: <%=path%>/<%=bimage2%></h3>
 <img src = "<%=conPath%>/bookImg/<%=bimage1%>" alt = '1번그림'>
 <img src = "<%=conPath%>/bookImg/<%=bimage2%>" alt = '2번그림'> 
 <b><a href ="<%=conPath%>/main/main.jsp">메인으로</a></b> 
</body>
</html>