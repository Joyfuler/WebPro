...
%>
<form action = "<%=conPath%>/Fileboard/updatePro.jsp?fid=<%=fid%>&pageNum=<%=pageNum%>" method = "post"
enctype = "multipart/form-data">
	<%-- 글작성의 경우 글 내용이 매우 길어지므로 get으로 보낼 수가 없음. 무조건 post를 사용해야 함. --%>
	<input type = "hidden" name = "cid" value = "<%=cDto.getCid()%>">
	<input type = "hidden" name = "fid" value = "<%=fid%>">
	<input type = "hidden" name = "pageNum" value = "<%=pageNum%>">
	<input type = "hidden" name = "dbFile" value = "<%=dto.getFilename()%>">
	<table>
		<caption>게시글수정</caption>
		<tr><th>작성자</th>
		<td><input type = "text" name = "fname" required = "required" autofocus="autofocus"
		value = "<%=cDto.getCname()%>" readonly = "readonly"></td>
		</tr>
		<tr><th>글제목</th>
		...
		value = "<%=cDto.getCemail()==null? "" : cDto.getCemail()%>" readonly = "readonly"></td>
		</tr>
		<tr><th>첨부파일</th>
		...
		<tr>
			<td colspan = "2">
				...
				<input type = "button" value = "게시글목록" class = "btn" onclick = "location.href='<%=conPath%>/Fileboard/csList.jsp?pageNum=<%=pageNum%>'">
			</td>
		</tr>
	</table>	
	</form>	
