...
<%} else if (result != null && result.equals("0")){%>
	...
	<tr onclick = "location.href='<%=conPath%>/Fileboard/writeForm.jsp'"><td id = "writeForm"><a href = "<%=conPath%>/Fileboard/writeForm.jsp">글쓰기</a></td></tr>			
	</table>
	<table>
	<tr><th>글번호</th><th>작성자</th><th>글제목</th><th>첨부파일</th><th>조회수</th>
	<%
	...
	for (FileBoardDto dtos: lists){		
		...
		<%}%>
		<a href = "<%=conPath%>/Fileboard/content.jsp?fid=<%=dtos.getFid()%>&pageNum=<%=pageNum%>"><%=dtos.getFtitle()%></a></td>		
		<%//메일
		String file = dtos.getFilename();
		out.print("<td>"+ (file==null? "-" : "<a href = '"+conPath+"/fileboardUpload/"+dtos.getFilename()+"'><img src = '"+conPath+"/img/fileup.jpg' height = '25'>" + "</a></td>"));		
		//조회수
