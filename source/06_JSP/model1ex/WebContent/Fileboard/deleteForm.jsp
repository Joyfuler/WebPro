				...
				<fieldset>
					...
					<form action="<%=conPath%>/Fileboard/deletePro.jsp" method="post">
						...
						<input type="hidden" name="cid" value="<%=cid%>"> <input
							type="hidden" name="fid" value="<%=fid%>">
						<p>
							암호 <input type="password" name="fpw" required="required"
								autofocus="autofocus" class="btn">
						</p>
						<input type="submit" value="삭제" class="btn">
					</form>
					<button
						onclick="location.href='<%=conPath%>/Fileboard/content.jsp?fid=<%=fid%>'">삭제취소</button>
				</fieldset>
			</td>
	</table>
