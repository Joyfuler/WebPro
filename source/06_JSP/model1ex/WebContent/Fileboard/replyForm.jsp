...
%>
원본 정보: <%=originalDto%>
수정하는사람 cid: <%=customer.getCid()%>
	<form action = "<%=conPath%>/Fileboard/replyPro.jsp?cid=<%=customer.getCid()%>" method = "post"
	enctype = "multipart/form-data">	
	<input type="hidden" name="pageNum" value="<%=pageNum %>">
	<input type="hidden" name="fid" value="<%=originalDto.getFid()%>"> 
	<input type="hidden" name="fgroup" value="<%=originalDto.getFgroup()%>">
	<input type="hidden" name="fstep" value="<%=originalDto.getFstep()%>"> 
	<input type="hidden" name="findent" value="<%=originalDto.getFindent()%>">
	<table>
	...
	<tr><th>글제목</th>
	<td><input type = "text" name = "ftitle" value = "re: <%=originalDto.getFtitle()%>" required = "required"></td>
	</tr>
	<tr><th>글내용</th>
