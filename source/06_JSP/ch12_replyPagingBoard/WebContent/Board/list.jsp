		...
		<tr>
			...
			if (totalCnt == 0) {
				...
				for (BoardDto dtos : lists) {
					...
					if (dtos.getBindent() > 0) { // 답변글인 경우는 이미지를 가지고 indent를 넣어줌. 이미지 넓이를 indent로 조절함.
				int width = dtos.getBindent() * 15;
			%>
			...
			%>
			<a
				href="<%=conPath%>/Board/content.jsp?bid=<%=dtos.getBid()%>&pageNum=<%=pageNum%>"><%=dtos.getBtitle()%></a>
			</td>
			<%
				//메일
