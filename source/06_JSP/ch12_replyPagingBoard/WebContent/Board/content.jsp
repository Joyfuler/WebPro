...
%>
	...
	<tr>
		<td colspan="2">
		<button onclick = "location.href='<%=conPath%>/Board/updateForm.jsp?bid=<%=bid%>&pageNum=<%=pageNum%>'">글수정</button>
		<button onclick = "location.href='<%=conPath%>/Board/deleteForm.jsp?bid=<%=bid%>&pageNum=<%=pageNum%>'">글삭제</button>
		<button onclick = "location.href='<%=conPath%>/Board/replyForm.jsp?bid=<%=bid%>&pageNum=<%=pageNum%>'">답변</button>
		<button onclick = "location.href ='<%=conPath%>/Board/list.jsp?pageNum=<%=pageNum%>'">글목록</button>
		</td>					
	</table>	 
<%}%>
</body>
