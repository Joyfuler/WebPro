...
세션에 데이터를 저장하기 위해서는 세션에 속성을 추가해야 함. (패러1 - 이름 패러2 - 값) --%>
	...
	%>
	<h2>
		session의 sessionName 속성값 :
		<%=session.getAttribute("sessionName")%></h2>
	<h2>
		session의 sessionNum 속성값 :
		<%=session.getAttribute("sessionNum")%></h2>
	<h2>
		session의 sessionMember 속성값 :
		<%=session.getAttribute("member")%></h2>
	<hr>
	<h2>
		세션 ID :
		<%=session.getId()%></h2>
