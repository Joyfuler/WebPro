...
<head>
  ...
  <script>
  $(function() {
	    // 파일 선택 input 엘리먼트와 미리보기용 img 엘리먼튼트를 가져옵니다.
	    const mphotoInput = $('#mphoto');
	    const previewImage = $('#previewImage');

	    // 파일 선택 input의 값이 변경될 때 호출되는 함수
	    mphotoInput.on('change', function () {
	        const selectedFile = mphotoInput[0].files[0];
	        if (selectedFile) {	            
	            ...
	            // 파일이 선택되지 않았을 경우 미리보기 이미지를 초기화합니다.
	            previewImage.attr('src', '${conPath }/memberPhotoUp/${member.mphoto}');
	        }
	    });
	});  
  </script>
				...
				<tr>
					...
					<td rowspan="4">						
						<img src="${conPath }/memberPhotoUp/${member.mphoto}"
								id = "previewImage" alt="${member.mname }사진">
					</td>
				</tr>
				<tr>
					<th>현비밀번호</th>
