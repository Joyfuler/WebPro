...
<body>
	...
	<table>
		...
		<tr>
			<td><%=bid%></td>
			<td><a href="<%=conPath%>/lect2_book/content.jsp?bid=<%=bid%>"><%=book.getBtitle()%></a></td>
			<td><a href="<%=conPath%>/lect2_book/content.jsp?bid=<%=bid%>"><img
					src='../bookImg/<%=book.getBimg1()%>' alt="대표이미지" width="15"></a></td>
			<td><del><%=price%></del><b><%=disPrice%>원(<%=discount%>%
					할인)</b></td>
			<td><%=discount%></td>
		</tr>
		<%
