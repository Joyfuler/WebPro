...
<body>
			...
			부서번호 <select name="deptno">
				...
				for (DeptDto dept : depts) { //확장for문. 타입  변수이름 : 배열
					...
					if (deptnoInput == deptno) {
						out.print("<option value = '" + deptno + "' selected = 'selected'>" + deptno + "-" + dname + "-" + loc
						+ "</option>");
					} else {
	...
	<table>
		...
		if (emps.isEmpty()) {
			...
			for (EmpDto emp : emps) {
				out.println("<tr><td>" + emp.getEmpno() + "</td><td>" + emp.getEname() + "</td><td>" + emp.getHiredate()
				+ "</td><td>" + emp.getDeptno() + "</td><td>" + emp.getDname() + "</td></tr>");
			}
