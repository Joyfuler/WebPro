...
public class Ex04_FriendMain {
	public static void main(String[] args) {
		...
		while (true) {
			System.out.print("검색할 생일을 00-00 형식으로 입력 (단, 종료를 원하시면 x를 입력하시오) : ");
			...
			for (Friend friend : friends) {
				Date birth = friend.getBirth(); // 입력할 때는 월-일만 입력.
				SimpleDateFormat sdf = new SimpleDateFormat("MM-dd");
				String birthStr = sdf.format(birth); // 월,일만 표기하는 String 변수를 만들어 줌.
				if (searchWord.equals(birthStr)) { // 월,일만 표기하는 String 변수의 값과 입력한 값이 같으면 됨.
