...
public class Ex04_FriendMain4 {
	public static void main(String[] args) {
		...
		while (true) {
			System.out.print("검색할 전화번호를 '전화번호 뒷자리(8888 등) 으로 입력 (단, 종료를 원하시면 x를 입력하시오) : ");
			...
			for (Friend friend : friends) {
				String phone = friend.getPhone(); // 입력할 때는 월-일만 입력.
				int idx = phone.lastIndexOf('-');
				if (phone.substring(idx + 1).equals(searchWord)) { 
					// 월,일만 표기하는 String 변수의 값과 입력한 값이 같으면 됨.
					System.out.println(friend);
					searchOk = true; // 찾으면 true로 변경함. 찾지 못한 경우 false인 채로.
				} // if

			} // for
			if (!searchOk) { // while 끝나기 전에 위치할 것.
				System.out.println("해당 전화번호를 가진 친구는 없습니다.");
			} // if
