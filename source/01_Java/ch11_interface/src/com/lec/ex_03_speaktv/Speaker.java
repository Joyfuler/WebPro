...
public class Speaker implements IVolume {
	...
	public void volumeUp(int level) {
		...
		if (volumeLevel + level < IVolume.SPEAKER_MAX_VOLUME) { // 현재 볼륨 + 5만큼 볼륨 올렸을 때 값이 최대볼륨보다 작다면 
			...
			int tempLevel = IVolume.SPEAKER_MAX_VOLUME - volumeLevel; // 임시레벨은 최대치에 근접했을 때 올릴 수 있는 볼륨. ex 현재 45, 최대 50이면 templevel =5
	...
	public void volumeDown(int level) {
		if (volumeLevel - level > SPEAKER_MIN_VOLUME) {
			...
			int tempLevel = (volumeLevel - SPEAKER_MIN_VOLUME); // 임시볼륨은 현재 볼륨에서 최소볼륨을 뺀 6. (현재 8인, 최소 6인데 10만큼 줄일 수 없으므로..)
