...
public class Account {
	...
	public void deposite(int money) { // 입금시
		...
		System.out.printf("%s : %d 원 예금하여, 잔액은 %d", accountNo, money, balance);

	}

	public void withdraw(int money) throws Exception { // 잔액이 있을 경우에만 인출이 가능. 잔액 부족시 떼끼 출력
		if (money <= balance) {
			balance -= money;
	...
	@Override
	public String toString() { // "101-1234", 홍길동님 잔액 : 1000원 데이터가 return됨. (String이므로 return값 필요)
  ...
  // getter & setter 추가. 단 잔액을 함부로 변조하지 않기 위해 balance는 set에서 제외. 계좌번호도 변경될 일이 없으니 username만 set
