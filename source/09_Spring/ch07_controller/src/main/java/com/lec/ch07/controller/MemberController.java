...
// member?method=join / login / logout / modify 요청경로는 동일, 패러미터만 다름
...
@Controller
@RequestMapping(value = "/member") // 공통 요청경로
public class MemberController {
	...
	public String modifyView(Model model) {
		model.addAttribute("member", new Member("aaa", "333"));
		return "member/modify";
	}
	
	@RequestMapping(params = "method=modify", method = RequestMethod.POST)
	public String modify(Model model) {
		model.addAttribute("kind", "Member_modify POST 수정완료"); 
		return "member/result";		
	}
